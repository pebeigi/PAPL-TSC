...
class MPLightAgent(RLAgent):
    ...
    def __init__(self, world, rank):
        ...
        # Pre-compute max observation length for padding (supports heterogeneous networks)
        self.max_ob_len = max(g.ob_length for _, g in self.ob_generator)
    ...
    def get_ob(self):
        ...
        :param: None
        :return x_obs: observation generated by ob_generator, shape [sub_agents, max_ob_len]
        '''
        x_obs = []
        for i in range(len(self.ob_generator)):
            tmp = self.ob_generator[i][1].generate()
            if self.ob_order is not None:
                tt = []
                name = self.ob_generator[i][1].I.id
                if name[:3] == 'GS_':
                    name = name[3:]
                for j in range(12):
                    if j in self.ob_order[name].keys():
                        tt.append(tmp[self.ob_order[name][j]])
                    else:
                        tt.append(0.)
                x_obs.append(np.array(tt, dtype=np.float32))
            else:
                # Pad to uniform length for heterogeneous networks
                padded = np.zeros(self.max_ob_len, dtype=np.float32)
                padded[:len(tmp)] = tmp
                x_obs.append(padded)
    ...
    def get_action(self, ob, phase, test=False):
        ...
        if self.phase:
            if self.one_hot:
                obs = np.concatenate([utils.idx2onehot(phase, self.num_actions), ob], axis=1)
            else:
                obs = np.array([np.concatenate([np.array([phase[i]]), ob[i]])
                                for i in range(self.sub_agents)], dtype=np.float32)
        else:
            obs = ob
        ...
        if self.valid_acts is None:
            ...
            # Build index→intersection_id map using ob_order if available, else world order
            if self.ob_order is not None:
                dic = {idx: key for idx, key in enumerate(self.ob_order.keys())}
            else:
                dic = {i: iid for i, iid in enumerate(self.world.intersection_ids)}
            def _lookup(d, i):
                k = dic[i]
                return d.get(k) or d.get('GS_' + k)
            batch_valid   = [_lookup(self.valid_acts,   i) for i in range(obs.shape[0])]
            batch_reverse = [_lookup(self.reverse_valid, i) for i in range(obs.shape[0])]
    ...
    def load_model_from_file(self, model_path):
        ...
        Load model weights from an explicit file path.

        :param model_path: full path to the .pt checkpoint file
        :return: None
        '''
        checkpoint = torch.load(model_path, map_location='cpu')
        # MPLight saves a dict with model_state_dict and optimizer_state_dict
...
class MPLight_InerAgent(DQN):
    ...
    def batch_act(self, batch_obs, valid_acts=None, reverse_valid=None, test=False):
        ...
        '''
        self.training = not test  # disable epsilon-greedy exploration during evaluation
