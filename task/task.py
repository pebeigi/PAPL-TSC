...
class TSCTask(BaseTask):
    ...
    def run(self):
        ...
        '''
        try:
            train = Registry.mapping['model_mapping']['setting'].param['train_model']
            if train:
                self.trainer.train()
            if Registry.mapping['model_mapping']['setting'].param['test_model']:
                # drop_load=True  → use model already in memory (just trained)
                # drop_load=False → load model weights from disk before testing
                self.trainer.test(drop_load=train)
